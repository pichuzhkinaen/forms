...
const form = document.getElementById('form'),
	  ...
	  formFile = document.querySelector("input[name='file']")
...
function checkLength(inputLength, input) {
	const inputParent = input.parentNode,
		  error = inputParent.querySelector('.error-text');
		
	if ( ! error && inputLength <= 1 ) {
		const message = ((inputLength === 1)) ? 'Имя должно быть не короче 2 символов' : '';
...
function checkEmail(inputValue, input) {
	...
	if ( ! error && ! inputValue.match(pattern)) {
		const message = 'Введите действительный адрес электронной почты';
			  input.classList.add('error');
...
// Проставление маски на полe ввода телефона
(function (){
	let code = '+7',
		find = /\+7/;
		code = '+7';
...
form.addEventListener('submit', function(e) {
	...
	if (document.querySelector('.error-text') || formName.value === '' || formEmail.value === '' || formFile.value === '') {
		return false;
	}
